...
def polynomial(x, y):
    ...
    def f(x, order=len(coefs)):
        total = coefs[0]
        for d in range(1,order):
            total = coefs[d] + (x - points[d]) * total
        return total
    f.points = points
    f.coefs = coefs
...
def polynomial_piece(left, right, interval=(0,1)):
    ...
    def f(x):
        ...
        total = coefs[0]
        for d in range(1,len(coefs)//2):
            total = coefs[d] + (x-1) * total
        for d in range(len(coefs)//2,len(coefs)):
            total = coefs[d] + x * total
        return total
    f.points = points
    f.coefs = coefs
...
if __name__ == "__main__":
    ...
    if TEST_POLY_APPROX:
        ...
        for (left, right) in left_rights:
            ...
            print("f.coefs:     ",f.coefs)
            print("f.points:    ",f.points)

            # TODO: Verify function by evaluating derivatives.
