...
\algorithm
    ...
    {where $x_0$, $x_1 \in \mathbb{R}$, $x_0 < x_1$, and $f$ is an order six polynomial. Returns \texttt{TRUE} if $f$ is monotone increasing on $[x_0,x_1]$.}
...
\algorithm
    ...
    {where $x_0,\ x_1 \in \mathbb{R}$, $x_0 < x_1$, and $f$ is an order six polynomial. Returns monotone $f$.}
    {\label{alg:make-monotone}
    \item if $\big(f(x_1) - f(x_0) = 0\big)$
    ...
    \item $Df(x_0) :=\ $median$\displaystyle \bigg(0,\ Df(x_0),\ 14 \frac{f(x_1) - f(x_0)}{x_1 - x_0} \bigg)$
      ...
      \comment{The selection of values in Step \ref{alg2-init-df} for $Df(x_0)$ and $Df(x_1)$ is suggested by \citeN{ulrich1994positivity} and originally from \citeN{huynh1993accurate}. These rules quickly enforce upper and lower bounds on derivative values to ensure quintic monotonicity is obtainable.}
      \comment{The next case that will be covered is in accordance with a simplified condition for quintic monotonicity that reduces to one of cubic positivity studied in \citeN{schmidt1988positivity}, where $\alpha$, $\beta$, $\gamma$, and $\delta$ are defined in terms of values and derivatives of $f$ at $x_0$ and $x_1$. Steps \ref{alg2-dx-conditions} and \ref{alg2-rescale-dx} ensure that the following bounds on $D^2f(x_0)$ are compatible. Step \ref{alg2-ddx0-upper} is the final preparatory step, ensuring that the conditions on $D^2f(x_1)$ are compatible. To conclude the simplified case, Step \ref{alg2-gamma} first enforces $\gamma \geq \delta$, then Steps \ref{alg2-alpha} and \ref{alg2-beta} enforce $\alpha > 0$ and $\beta \geq \alpha$ respectively. Combined, these three conditions derived from Equation (2.18) in \citeN{schmidt1988positivity} guarantee monotonicity of this polynomial piece.}
    \item if $\big(Df(x_0) = 0$ or $Df(x_1) = 0\big)$
    \item \indented{$w := x_0 - x_1$\\$v := f(x_0) - f(x_1)$}
    ...
    \item if $\big($max$(A,B) > 6\big)$
      \indented{$Df(x_0) := 6 Df(x_0)\ /\ $max$(A,B)$}
      \indented{$Df(x_1) := 6 Df(x_1)\ /\ $max$(A,B)$}
      \comment{This box bound ensures that $(A,B)$ remains within a viable region of monotonicity (satisfying Theorem 4, seen in Fig. 6 of \citeN{ulrich1994positivity}).}
    ...
    \item $D^2f(x_0),\ D^2f(x_1) := $ binary\_search$\big($is\_monotone$(x_0,x_1,f)$, $\eta,$ $\eta_0\big)$
      \comment{This binary search identifies feasible values of $D^2f(x_0)$ and $D^2f(x_1)$ that are nearest to the current values. This search is valid because $\eta_0$ is guaranteed to produce a monotone function, which can be seen in Equation (23) of \citeN{ulrich1994positivity}.}
    \item return $f$
}
%% ----------------------------------------------------------------------
...
\algorithm
    ...
    {where $\{k_1,\ldots,k_n\}$ is an increasing sequence of real numbers, $f$ is a piecewise order six polynomial defined over intervals $[k_i, k_{i+1}]$  for $i = 1, \ldots, n-1$, and integer $s \geq 1$.}
    {\label{alg:monotone-spline}
    \item create queue
      \\ create changed$(1, \ldots, n) := $ FALSE
      \\ create step\_size$(1, \ldots, n) := \{Df(k_i) / s$ for $i = 1, \ldots, n\}$\label{step:create}
      \comment{The three variables defined in Step \ref{step:create} are used to ensure the eventual achievement of monotonicity. The \texttt{queue} is a standard first in first out (FIFO) queue with \texttt{enqueue} and \texttt{dequeue} operations. The array \texttt{changed} contains Booleans describing whether or not a partition point belongs to an interval that has been modified to enforce monotonicity. The \texttt{step\_size} array contains the real valued derivative step sizes to use in the search for a valid monotone spline.}
    \item for $i := 1, \ldots, n-1$
      \indented{enqueue $(k_i, k_{i+1})$}
    ...
    \item \indented{\indented{if $\big($changed$(j)\big) \qquad \ Df(x_j) := Df(x_j) - $step\_size$(j)$}}
      ...
      \indented{\indented{\comment{If a partition point belongs to an interval that has been previously modified, then the enforced monotonicity conditions on the second derivatives of adjacent intervals must contradict one another. In turn, the involved first derivatives are decreased towards zero by a predetermined step size.}}}
    ...
    \item \indented{\indented{if $\big(j > 1$ and $(x_{j-1}, x_j) \not \in$ queue$\big)$ enqueue $(x_{j-1}, x_j)$}}
      \indented{\indented{if $\big((x_j, x_{j+1}) \not \in$ queue$\big)$ enqueue $(x_j, x_{j+1})$}}
      \indented{\indented{if $\big(j+1 < n$ and $(x_{j+1}, x_{j+2}) \not \in$ queue$\big)$ enqueue $(x_{j+1}, x_{j+2})$}} \label{step:requeue}
