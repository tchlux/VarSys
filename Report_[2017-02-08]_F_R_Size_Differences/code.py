...
import pygal
import numpy as np
from scipy.interpolate import CubicSpline, splrep, splev, PchipInterpolator
...
<html lang="en">
  ...
  <body id="Body">
	...
	<strong>Scatter Plot of Variance for 2 Independent 40 Runs</strong>
      ...
      <div id="Variance-Scatter" class="collapse" aria-expanded="false">
      </div>
    </div>
...
# python3 code.py >> report_smoothed.html && google-chrome report_smoothed.html

def smooth_cdf_plot(data1, data2, file_name):
    # Use python lists to sort since numpy gives a deprecation warning
    data1 = list(data1)
    data1.sort()
    ...
    # Get the minimum and maximum points from the data (for edges of CDF)
    min_pt = min(min(data1), min(data2))
    max_pt = max(max(data1), max(data2))
    ...
    # Add all of the CDF points for the two data sets
    ...
    data1_pts = np.array(data1_pts)
    data2_pts = np.array(data2_pts)

    # Create spline fits of the CDF points
    fit_1 = PchipInterpolator(data1_pts[:,0], data1_pts[:,1])
    fit_2 = PchipInterpolator(data2_pts[:,0], data2_pts[:,1])

    # fit_1 = CubicSpline(data1_pts[:,0], data1_pts[:,1], bc_type="clamped")
    # fit_2 = CubicSpline(data2_pts[:,0], data2_pts[:,1], bc_type="clamped")

    # fit_1_sp = splrep(data1_pts[:,0], data1_pts[:,1], k=3)
    # fit_2_sp = splrep(data2_pts[:,0], data2_pts[:,1], k=3)
    # fit_1 = lambda x: splev(x, fit_1_sp)
    # fit_2 = lambda x: splev(x, fit_2_sp)

    # Create the plot
    config = pygal.Config(stroke=True, fill=False, show_legend=True, show_dots=False)
    ...
    # print(x_points)
    # x_points = data1_pts[:,0]
    # print(x_points)

    fit_data1_points = np.array(list(zip(x_points, fit_1(x_points))))
    # x_points = data2_pts[:,0]
    fit_data2_points = np.array(list(zip(x_points, fit_2(x_points))))

    fit_data1_points[np.where(fit_data1_points[:,0] <= min(data1)),1] = 0.0
    fit_data1_points[np.where(fit_data1_points[:,0] >= max(data1)),1] = 100.0
    fit_data2_points[np.where(fit_data2_points[:,0] <= min(data2)),1] = 0.0
    fit_data2_points[np.where(fit_data2_points[:,0] >= max(data2)),1] = 100.0

    fit_data1_points[np.where(fit_data1_points[:,1] < 0),1] = 0.0
    fit_data2_points[np.where(fit_data2_points[:,1] < 0),1] = 0.0
    fit_data1_points[np.where(fit_data1_points[:,1] > 100),1] = 100.0
    fit_data2_points[np.where(fit_data2_points[:,1] > 100),1] = 100.0

    # print(data1_pts)
    # print(fit_data1_points)
    # print(fit_data2_points)

    chart.add("Old 40", data1_pts)
    chart.add("New 40", data2_pts)
    chart.add("Fit Old 40", fit_data1_points)
    chart.add("Fit New 40", fit_data2_points)
    chart.render_to_file(file_name)    
...
def cdf_plot(data1, data2, file_name):
    ...
    # Create the plot
    config = pygal.Config(fill=False, show_legend=True, shot_dots=False)
    chart = pygal.XY(config)
...
if __name__ == "__main__":
    ...
    for f_size, r_size in f_r_sizes:
        ...
        smooth_cdf_plot(data_old, data_new, file_name)
