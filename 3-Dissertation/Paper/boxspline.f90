...
SUBROUTINE BOXSPLEV(UNIQUE_DVECS, DVEC_MULTS, EVAL_PTS, BOX_EVALS, ERROR)
  ...
  INTEGER :: D, NUM_DVECS, NUM_PTS, DEPTH, INFO
  ...
  REAL(KIND=REAL64), DIMENSION(SIZE(EVAL_PTS,2),                          &
       SUM(DVEC_MULTS)-SIZE(UNIQUE_DVECS,1)+2) :: EVALS_AT_PTS
  REAL(KIND=REAL64), DIMENSION(SIZE(UNIQUE_DVECS,2),SIZE(EVAL_PTS,2),     &
       SUM(DVEC_MULTS)-SIZE(UNIQUE_DVECS,1)+2) :: SHIFTED_EVAL_PTS
  ! ------------------------------------------------------------------
  ...
  IF (SIZE(EVAL_PTS,1)   .NE. D)         THEN; ERROR = 2; RETURN; END IF
  ...
  CALL EVALUATE_BOX_SPLINE(&
       DVECS(:,:,1), LOC(:,1), MULTS(:,1), SHIFTED_EVAL_PTS(:,:,1), &
       EVALS_AT_PTS(:,1), &
...
CONTAINS
  ...
  RECURSIVE SUBROUTINE EVALUATE_BOX_SPLINE(&
    ...
    REAL(KIND=REAL64), INTENT(OUT), DIMENSION(:)   :: R_EVALS_AT_PTS
    REAL(KIND=REAL64), INTENT(OUT), DIMENSION(:)   :: R_NEXT_EVALS_AT_PTS
    ! Local variables
    ...
    ELSE
       ...
       compute_shift_4 : DO IDX_1 = 1, NUM_PTS
          R_NEXT_SHIFTED_EVAL_PTS(:D,IDX_1) = EVAL_PTS(:,IDX_1) - PT_SHIFT(:)
       ...
       DO IDX_1 = 1, D
          ...
          ! Compute shifted evaluation locations. (1, D) x (D, NUM_PTS)
          R_NEXT_EVALS_AT_PTS(:) = MATMUL(NORMAL_VECTOR, &
               R_NEXT_SHIFTED_EVAL_PTS(:D,:))
