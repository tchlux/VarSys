...
CONTAINS
  ...
  SUBROUTINE max_boxes(pts, box_widths)
    ...
    REAL(KIND=R8), DIMENSION(UBOUND(pts,2)) :: distances
    ...
    LOGICAL, DIMENSION(UBOUND(pts,1)) :: undefined_low, undefined_upp
    REAL(KIND=R8), DIMENSION(UBOUND(pts,1)) :: low_width, upp_width, &
         box_center, point
    ...
    compute_box_shapes : DO step = 1,UBOUND(pts,2)
       ! Initialize the width of this box to be undefined
       box_center = pts(:,step)
       low_width = -1.0_R8
       upp_width = -1.0_R8
       undefined_low = .TRUE.
       ...
       linf_distances : DO CONCURRENT (idx = 1: UBOUND(pts,2))
          distances(idx) = MAXVAL(ABS(pts(:,idx) - box_center))
       ...
       calculate_box_width : DO other_pt = 1, UBOUND(pts,2)
          ...
          ! Store the other point (take better advantage of caches)
          point = pts(:,other_pt_idx)

          ! Logical testing if a point is inside a box:
          !  -- lower width undefined .OR. (box - other) < lower width
          in_lower = (undefined_low) .OR. (box_center-point < low_width)

          !  -- upper width undefined .OR. (other - box) < upper width
          in_upper = (undefined_upp) .OR. (point-box_center < upp_width)
          ...
          IF (ALL(in_lower .AND. in_upper)) THEN
             ...
             dim = MAXLOC(ABS(point - box_center),1)
             ! Check if we are adjusting the upper or lower width of
             ! the box, and adjust the boundary for this point
             IF (box_center(dim) > point(dim)) THEN
                low_width(dim) = box_center(dim) - point(dim)
                undefined_low(dim) = .FALSE.
             ELSE
                upp_width(dim) = point(dim) - box_center(dim)
                undefined_upp(dim) = .FALSE.
             END IF
          END IF
       END DO calculate_box_width
       box_widths(:UBOUND(pts,1),step) = low_width
       box_widths(UBOUND(pts,1)+1:,step) = upp_width
  ...
  SUBROUTINE linear_eval(boxes, box_widths, box_values, pts, values)
    ! Box point coordinates
    REAL(KIND=R8), INTENT(IN), DIMENSION(:,:)               :: boxes
    ...
    REAL(KIND=R8), INTENT(IN), DIMENSION(UBOUND(boxes,2))   :: box_values
    ! The new points to evaluate the max-box-mesh at
    REAL(KIND=R8), INTENT(IN), DIMENSION(:,:) :: pts
    ! The holder for returning the values at each of the new points
    REAL(KIND=R8), INTENT(OUT), DIMENSION(UBOUND(pts,2))    :: values
    ! Local variables for evaluating all of the boxes
    INTEGER :: pt_num, box_num, dim
    REAL(KIND=R8) :: val, divisor
    REAL(KIND=R8), DIMENSION(UBOUND(boxes,1)) :: shifted_box
    ...
    pt_evals : DO pt_num = 1,UBOUND(pts,2)
       ...
       box_evals : DO box_num = 1,UBOUND(boxes,2)
          ...
          normal_box : DO dim = 1,UBOUND(boxes,1)
             ...
             IF (boxes(dim,box_num) > pts(dim,pt_num)) THEN
             ...
             IF (shifted_box(dim) < 0) shifted_box(dim) = 0.0_R8
          END DO normal_box

          ! ASSERT: "shifted_box" is not the shifted x-coordinate
