...
C
C     F -- (input) double precision array of dependent variable
C           values to be interpolated.  F(I) is value corresponding
C           to X(I).
C
C     D -- (output) double precision array of derivative values
...
C           are computed from the derivative of a local quadratic
C           unless this alters monotonicity.
C
C     WK -- (scratch) real work array, which must be declared by the
C           calling program to be at least 2*N if SPLINE is .TRUE.
C           and not used otherwise.
C
...
C              IERR = 0  (no errors).
C           Warning error:
C              IERR.GT.0  (can only occur when SPLINE=.FALSE.) means
C                 that IERR switches in the direction of monotonicity
C                 were detected.  When SPLINE=.FALSE.,  DPCHEZ guarantees
c                 that if the input data is monotone, the interpolant
C                 will be too. This warning is to alert you to the fact
C                 that the input data was not monotone.
