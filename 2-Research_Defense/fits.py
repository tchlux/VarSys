...
def flat_fit(data):
    ...
    min_value = min(x)
    def edf(z):
        try: return list(map(edf, z))
        except:
            if (z < min_value): return 0
            return y[min(len(y)-1, max(0,searchsorted(x, z, side="left")-1)
                         + (1 if z == max(x) else 0))]
    return edf
...
def quintic_fit(data):
    ...
    x, y = cdf_points(data)
    # Add a left-most CDF point.
    from fraction import Fraction
    x.insert(0,Fraction(-2.447))
    y.insert(0,Fraction(0))
    x = list(map(float,x))
    y = list(map(float,y))
    # exit()
