...
SUBROUTINE BOXSPLEV(DVECS, DVEC_MULTS, EVAL_PTS, BOX_EVALS, ERROR)
  ...
  PRINT *, "DIM               : ", DIM
  PRINT *, "NUM_DVECS         : ", NUM_DVECS
  PRINT *, "SHAPE(IDENTITY)   : ", SHAPE(IDENTITY)
  PRINT *, "SIZE(LAPACK_WORK) : ", SIZE(LAPACK_WORK)
  PRINT *, "SIZE(DVEC_MULTS)  : ", SIZE(DVEC_MULTS)
  ...
  ELSE   
     ...
     RCOND = MATRIX_CONDITION_INV(DVECS(1:SIZE(DVECS,1), 1:SIZE(DVECS,1)))
...
CONTAINS
       ...
       DO IDX_1 = 1, NUM_DVECS
          ...
          IF (MULTS(IDX_1) .GT. 1) THEN
             ...
             compute_shift_1 : DO IDX_3 = 1, SIZE(SUB_DVECS,2)
                TEMP_SHIFTED_EVAL_PTS(:,IDX_3) = SHIFTED_EVAL_PTS(:,IDX_3) - SUM(DVECS(:,IDX_1) * SUB_DVECS(:,IDX_3))
  ...
  FUNCTION MATRIX_MINIMUM_NORM(MATRIX) RESULT(MIN_NORM)
    ...
    !   LAPACK_WORK_SIZE -- Size of dgels_work_array
  ...
  FUNCTION ALLOCATE_MAX_LAPACK_WORK() RESULT(WORK)
    ...
    CALL DGESVD('N', 'A', NUM_DVECS, DIM, TRANSPOSE(DVECS), NUM_DVECS, &
         TRANSPOSE(DVECS), WORK, 1, MATMUL(DVECS, TRANSPOSE(DVECS)), &
         NUM_DVECS, SIZES, -1, INFO)
    PRINT *, "SIZES: ", SIZES, INFO

    ! Query the size of the work array to (MATRIX_RANK)
    CALL DGESVD('N', 'N', NUM_DVECS, DIM, TRANSPOSE(DVECS), NUM_DVECS, &
         TRANSPOSE(DVECS), WORK, 1, WORK, 1, SIZES, -1, INFO)
    PRINT *, "SIZES: ", SIZES, INFO
    ...
    ! 
    ! MATRIX_MINIMUM_NORM work size:          130
    ! MATRIX_ORTHOGONAL work size:           68
    ! MATRIX_ORTHOGONAL work size:           68
    ! MATRIX_ORTHOGONAL work size:           68
    ! MATRIX_ORTHOGONAL work size:           68
    ! MATRIX_RANK work size:          134
    ! MATRIX_MINIMUM_NORM work size:           98
    ! MATRIX_RANK work size:          134
    ! MATRIX_MINIMUM_NORM work size:           66
