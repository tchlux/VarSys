...
SUBROUTINE BOXSPLEV(DVECS, DVEC_MULTS, EVAL_PTS, BOX_EVALS, ERROR)
  ...
  CALL EVALUATE_BOX_SPLINE(DVEC_MULTS, LOCATION, MIN_NORM_DVECS, &
       MATMUL(TRANSPOSE(MIN_NORM_DVECS), EVAL_PTS), BOX_EVALS)
  IF (ERROR .NE. 0) RETURN
...
CONTAINS
  ...
  RECURSIVE SUBROUTINE EVALUATE_BOX_SPLINE(MULTS, LOC, SUB_DVECS,&
    ...
    REAL(KIND=R8), INTENT(OUT), DIMENSION(SIZE(EVAL_PTS,2)) :: EVALS_AT_PTS
    ...
    REAL(KIND=R8), DIMENSION(SIZE(EVAL_PTS,2))  :: TEMP_EVALS_AT_PTS
    REAL(KIND=R8), DIMENSION(SIZE(EVAL_PTS,2))  :: LOCATIONS
    ...
    REAL(KIND=R8), DIMENSION(SIZE(SUB_DVECS,2)) :: TEMP_PT_SHIFT
    ...
    IF (SUM(MULTS) > DIM) THEN
       ...
       DO IDX_1 = 1, NUM_DVECS
          ...
          NEXT_LOC(:)   = LOC(:)   + IDENTITY(:, IDX_1) ! Track location in recursion.
          ...
          IF (MULTS(IDX_1) .GT. 1) THEN
             ...
             FORALL (IDX_3 = 1:SIZE(SUB_DVECS,2))
                TEMP_PT_SHIFT(IDX_3) = SUM(DVECS(:,IDX_1) * SUB_DVECS(:,IDX_3))
             END FORALL
             compute_shift_1 : FORALL (IDX_3 = 1:NUM_PTS)
                TEMP_SHIFTED_EVAL_PTS(:,IDX_3) = SHIFTED_EVAL_PTS(:,IDX_3) - TEMP_PT_SHIFT(:)
          ...
          ELSE IF (MULTS(IDX_1) .GT. 0) THEN
             ...
             IF (MATRIX_RANK(TRANSPOSE(NEXT_DVECS)) .EQ. DIM) THEN
                ...
                END FORALL
                compute_shift_2 : FORALL (IDX_3 = 1:NUM_PTS)
                ...
                END FORALL
                compute_shift_3 : FORALL (IDX_3 = 1:NUM_PTS)
                   TEMP_EVAL_PTS(:,IDX_3) = EVAL_PTS(:,IDX_3) - PT_SHIFT(:)
    ...
    ELSE
       ...
       END FORALL
       compute_shift_4 : FORALL (IDX_1 = 1:NUM_PTS)
          TEMP_EVAL_PTS(:,IDX_1) = EVAL_PTS(:,IDX_1) - PT_SHIFT(:)
  ...
  FUNCTION MATRIX_MINIMUM_NORM(MATRIX) RESULT(MIN_NORM)
    ...
    MIN_NORM(:,:) = 0_R8
    FORALL (IDX = 1:SIZE(MIN_NORM,1)) MIN_NORM(IDX,IDX) = 1_R8
