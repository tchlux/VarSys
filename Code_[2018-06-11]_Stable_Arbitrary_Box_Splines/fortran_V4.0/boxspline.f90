...
CONTAINS
    ...
    IF (SUM(MULTS) > DIM) THEN
       ...
       DO IDX_1 = 1, NUM_DVECS
          ...
          ELSE IF (MULTS(IDX_1) .GT. 0) THEN
             ...
             IF (ERROR .NE. 0) RETURN
             IF (FULL_RANK(TRANSPOSE(NEXT_DVECS))) THEN
                IF (ERROR .NE. 0) RETURN
  ...
  SUBROUTINE MATRIX_ORTHOGONAL(A, ORTHOGONAL)
    ...
    ! Check for a guaranteed singular vector.
    IF (SIZE(VT,1) > SIZE(S)) THEN
       ORTHOGONAL = VT(SIZE(S)+1,:)
       RETURN
    END IF
    ! Otherwise, look for an orthogonal vector as last vector.
    ORTHOGONAL = 0.
    TOL = EPSILON(1.0_REAL64) * DIM * S(1)
    ! Check for a vector in the orthonormal basis for the null
    ! space of A (a vector with an extremely small singular value).
    IF (S(SIZE(S)) .LE. TOL) THEN
       ORTHOGONAL = VT(SIZE(S),:)
    END IF
  ...
  FUNCTION FULL_RANK(A)
    ...
    REAL(KIND=REAL64), DIMENSION(MINVAL(SHAPE(A))) :: S
    INTEGER :: IDX
    LOGICAL :: FULL_RANK
    ! Unused DGESVD parameters.
    REAL(KIND=REAL64), DIMENSION(1) :: U, VT
    ! Check to see if the direction vectors certainly do not have full rank.
    IF (SIZE(S) .LT. DIM) THEN; FULL_RANK = .FALSE.; RETURN; END IF
    ! Use the SVD to get the orthogonal vectors.
    ...
    ! Check to see if this set of direction vectors has full rank by
    ! checking if the "DIM" singular value is larger than a reasonable
    ! singular value tolerance based on expected numerical error 
    ! ["Numerical Recipes" by W. H. Press et al., Matlab].
    FULL_RANK = S(DIM) .GT. (EPSILON(1.0_REAL64) * DIM * S(1))
