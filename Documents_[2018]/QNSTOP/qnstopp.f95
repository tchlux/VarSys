...
REAL(KIND=R8):: Mu_d ! Constant for the trust region subproblem.
...
REAL(KIND=R8), PARAMETER:: Mu_c = 0.0_R8 ! For \mu update formula.
REAL(KIND=R8), PARAMETER:: Tau_b = 7.0_R8/16.0_R8 ! Experimental design
! region and trust region radius.
...
REAL(KIND=R8), DIMENSION(:), POINTER:: HatGradient
REAL(KIND=R8), DIMENSION(:), POINTER:: PrevGradient

REAL(KIND=R8), DIMENSION(:,:), POINTER:: D_kT
REAL(KIND=R8), DIMENSION(:,:), POINTER:: HatHessian
...
END IF
IF ((.NOT. PRESENT(MAX_ITER)) .AND. (.NOT. PRESENT(MAX_EVAL)) .AND. &
    ((LMIN_TAU .EQ. 0.0_R8) .OR. &
    ((LMIN_TAU > 0.0_R8) .AND. (ANY(MODE .EQ. (/'G','g'/)))))) THEN
...
!have a private copy of.
!$OMP& PRIVATE(D_kT,EigVal,Gradient,HatGradient,HatHessian,IWork, &
!$OMP&   IWork_2D,K,Mu,PrevGradient, &
...
!$OMP&   t_k,ThreadNum,XiUpdate,XValues,YValues) &
!
...
!   U2 = {{\hat H_{k-1}} s_k s_k^T {\hat H_{k-1}}}/{s_k^T {\hat H_{k-1}} s_k}.
    ...
    CALL DSYMV('U', P, 1.0_R8, Hessian, P, XiUpdate, 1, 0.0_R8, &
      RWork1D_3(1:P), 1)
! RWork1D_3 = H_{k-1} \s_k.
    CALL DSYR('U', P, -1.0_R8/DOT_PRODUCT(RWork1D_3(1:P), XiUpdate), &
      RWork1D_3(1:P), 1, Hessian, P)
! Hessian = Hessian - U2.
    CALL DSYR('U', P, 1.0_R8/DOT_PRODUCT(RWork1D_2, XiUpdate), RWork1D_2, 1, &
      Hessian, P)
...
!
!   H_{k+1} = H_k + z_k z_k^T / (z_k^T s_k),
!
! where z_k = \nu_k - H_k s_k.
      RWork1D_1 = RWork1D_2
...
! Choose the update U_i such that ||(H_k+U_i)s_k - \nu_k|| is minimal.
...
!    a. (MODE 'G') Compute \mu_k by solving
...
! Compute sqrt(W^{-1}) = v sqrt(\lambda) v^T,
...
! Compute {\hat H_k} = sqrt(W^{-1}) H sqrt(W^{-1}).
    ...
    HatHessian => RWork2D_6(1:P,1:P) ! Scaled Hessian \hat H_k.

! Compute {\hat g_k} = sqrt(W^{-1}) g_k.
    CALL DSYMV('U', P, 1.0_R8, RWork2D_1, P, Gradient, 1, 0.0_R8, &
      RWork1D_1, 1)
    HatGradient => RWork1D_1(1:P) ! Scaled gradient \hat g_k.

! Let \lambda_1 be the smallest eigenvalue of {\hat H_k}.
! If \lambda_1 < 0, set \mu_k = -Plus \lambda_1.
! Otherwise, set \mu_k = 0.
    RWork2D_4(1:P,1:P) = HatHessian
...
! b is the solution to the linear system L X = - {\hat g_k}.
...
! Compute the Cholesky factorization L L^T = \hat H_k + \mu I.
        RWork2D_2(1:P,1:P) = HatHessian
...
! c is the solution to the linear system L x = -{\hat g_k}.
        RWork1D_4(1:P) = -HatGradient
        RWork2D_3(1:P,1:P) = RWork2D_2(1:P,1:P)
...
! s = RWork1D_4.
      ...
      END DO
    END IF
    NULLIFY(HatHessian, HatGradient)
  ELSE
    Mu = Mu_d*(Mu_c + REAL(CurrentIter, KIND=R8))
  END IF

! Step 4b.
! Compute the step and update \xi_k by solving the linear system
!
!   ({\hat H_k} + \mu_k W_k) \xi_{k+1} = -{\hat g_k}.
!
  XiUpdate = -Gradient
...
!
  ...
  RWork_1 = DOT_PRODUCT(RWork1D_3(1:LN), RWork1D_3(1:LN))/(LN-(P+1))
! RWork_1 = \sigma^2.
