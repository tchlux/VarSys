...
\begin{proofdot}
  ...
  \begin{align*}
	...
	&\leq \frac{\gamma}{2} \|x_j - x_0\|_2^2 \leq \frac{\gamma k^2}{2}, \\
  \end{align*}
  for all $1 \leq j \leq d.$ Using {\it Lemma \ref{lemma:2}},
  $$\big \| \nabla f(x_i) - c \big \|_2^2 \leq \frac{d}{\sigma_d^2}
  \big( \gamma k^2 / 2 \big)^2 \implies \big \| \nabla f(x_i) - c \big \|_2
  \leq \sqrt{d} \frac{\gamma k^2}{2 \sigma_d}.$$
  \qed
\end{proofdot}
...
The theoretical error bound is tight and that can be observed with the
test function $q(x) = \|x\|_2^2,$ $x \in \mathbb{R}^d$ for $d \in
\mathcal{N},$ and the simplex defined by vertices $X = \{\mathbf{0}, e_1,
\ldots, e_d\}$ where $e_i$ is $i$-th unit vector in $\mathbb{R}^d.$
The relevant constants to the error bound are

\vspace{-2mm}
$$ \gamma = 2, \qquad \sigma_d = 1, \qquad k = 1, \qquad x_0 =
\mathbf{0}, \qquad \hat q(x) = \langle\mathbf{1}, x - \mathbf{0}\rangle +
q(\mathbf{0}). $$
\vspace{-2mm}

\noindent Noting that $q(\mathbf{0}) = 0,$ the approximation error at $z = -\mathbf{1/2}$ is

\vspace{-2mm}
\begin{align*}
  \big|q(z) - \hat q(z)\big| \quad
  =& \quad \big|\|z\|_2^2 - \langle\mathbf{1},z\rangle\big| \\
  =& \quad \bigg| \frac{d}{4} + \frac{d}{2} \bigg| \quad = \quad \frac{3d}{4},
\end{align*}
\vspace{-2mm}

\noindent while plugging the constants into the error bound from the theorem gives

\vspace{-2mm}
\begin{align*}
  \big|q(z) - \hat q(z)\big| \quad
  \leq& \quad \frac{\gamma \|z - x_0\|_2^2}{2} + \sqrt{d} \frac{\gamma k^2}
      {2 \sigma_d} \|z - x_0\|_2 \\
  =& \quad \|z\|_2^2 + \sqrt{d} \|z\|_2 \\
  =& \quad \frac{d}{4} + \frac{d}{2} \quad = \quad \frac{3d}{4}.
\end{align*}
\vspace{-2mm}

\noindent In light of this error bound, an empirical evaluation of
presented interpolation and regression algorithms follows.


%% ===================================================================
\vspace{-2mm}
\section{Analytic Test of Convergence}
\label{sec:analytic}
...
test the error of interpolants and regressors given an increasing
number of examples and increasing relative random error at data
points.
...
\begin{figure}
  \centering
  \begin{tikzpicture}
    \node (img)  {\includegraphics[width=0.75\textwidth,]{Fig2.pdf}};
...
\begin{figure}
  \centering
  \begin{tikzpicture}
    \node (img)  {\includegraphics[width=0.8\textwidth,]{Fig3.pdf}};
...
\begin{figure}
  \centering
  \begin{tikzpicture}
    \node (img)  {\includegraphics[width=0.8\textwidth]{Fig12.pdf}};
