...
DATA_PATH = os.path.join(HOME_DIR, *["Git", "VarSys",
                                     "Report_[2018-11-06]_MOANA_Figure_Update",
                                     "MODIFIED-wendy-virtual.csv"])
...
if PLOT_CONVERGENCE:
    ...
    vm_sched = "NOOP"
    test_type = "Fread"
    # Picking the configuration to predict
    file_size   = [200000]
    record_size = [16384]
    threads     = [3]
    frequencies = [2100000]
    ...
    guessed_p = Plot("90% Confidence Interval with Increasing Sample Size (Large File Size, 80 Samples Max)", 
    ...
    # Store temporary copies before plotting
    orig_lower = lower.copy()
    orig_guessed_lower = guessed_lower.copy()
    orig_upper = upper.copy()
    orig_guessed_upper = guessed_upper.copy()

    # Fix lower, guessed_lower, upper, and guessed_upper
    for i in range(1,len(lower)):
        lower[i] = max(lower[i], lower[i-1])
    for i in range(1,len(guessed_lower)):
        guessed_lower[i] = max(guessed_lower[i], guessed_lower[i-1])
    for i in range(1,len(upper)):
        upper[i] = min(upper[i], upper[i-1])
    for i in range(1,len(guessed_upper)):
        guessed_upper[i] = min(guessed_upper[i], guessed_upper[i-1])
    ...
    # Undo the fixes made for the plot.
    lower = orig_lower.copy()
    guessed_lower = orig_guessed_lower.copy()
    upper = orig_upper.copy()
    guessed_upper = orig_guessed_upper.copy()
